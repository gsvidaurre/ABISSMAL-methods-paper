...
We planned and developed the code for validation analyses with a small random subsample of all of the videos used for this analysis.
...
# Get all of the unique BORIS Observation IDs that correspond to unique recording events
...
## Choosing a random subsample of videos for code development

We chose a subsample of these videos to develop the validation analysis pipeline.
```{r eval = FALSE}
...
## Remove flipped videos

Summer noticed some video recording events had videos switched in order, so we have to pre-process the manually scored dataset to fix these issues for the videos she caught that had this issue. Look at the spreadsheet called "BORIS_VideoList_TimeFlipped.csv". We proceeded by excluding these videos from analyses. 

TKTK need to check with Summer about whether the numeric IDs in the column of this spreadsheet are row indices or BORIS observation IDs. Will proceed for now as the latter.
```{r}

time_flipped <- read.csv(file.path(path, "BORIS_VideoList_TimeFlipped.csv"))
glimpse(time_flipped)

manual_scoring2 <- manual_scoring %>% 
  dplyr::filter(!grepl(paste(paste("_", time_flipped[["Flipped"]], "$", sep = ""), collapse = "|"), Observation.id))

glimpse(manual_scoring2)

# 33 rows deleted
nrow(manual_scoring2) - nrow(manual_scoring) 

# 15 rows here
nrow(time_flipped)

# Checking: This discrepancy is because each row of manual_scoring is not a unique video but rather a unique behavioral state per video, looks good
manual_scoring$Observation.id[grepl(paste(paste("_", time_flipped[["Flipped"]], "$", sep = ""), collapse = "|"), manual_scoring$Observation.id)]

time_flipped[["Flipped"]]

```

Some observations are missing two media duration values, which is an indication that something went wrong with one of the videos in the unique recording event. Remove these observations as well. 
```{r}

# The two media file duration values are separated by a semi-colon in each row
# Three observations had this issue in the full dataset
which(!grepl(";", manual_scoring2$Media.duration..s.))

# These three observations were scored across two unique recording events
manual_scoring2$Media.file.name[which(!grepl(";", manual_scoring2$Media.duration..s.))]

manual_scoring2 <- manual_scoring2 %>%
  dplyr::filter(grepl(";", manual_scoring2$Media.duration..s.))

glimpse(manual_scoring2)

```

## Notes on videos recorded during daily health checks

We did not remove videos for which video recording was initialized by daily animal checks (a hand opening the nest container). These videos should have been scored as no birds inside of the container, which is part of our ethogram. These videos could be filtered out for other purposes by checking for a no birds score and a large number of pixels that led to the video recording. Summer kept track in her video scoring log as well, but removing these videos was not critical for the analyses here.
```{r}

glimpse(manual_scoring2)

# No useful info here about daily health checks
unique(manual_scoring2$Comment.start)
unique(manual_scoring2$Comment.stop)
unique(manual_scoring2$Description)

```

## Unmask the video file names used for manual scoring

Add back the original video file names to the manually scored data that was filtered to remove time-flipped video recording events.
```{r}

# Add the unmasked video file names to each observation in the manual scoring spreadsheet
# i <- 1 # testing
manual_scoring_nm <- data.table::rbindlist(pblapply(1:nrow(manual_scoring2), function(i){
  
  # Get the masked video recording event ID
  masked_nm <- manual_scoring2$masked_video_recording_event_ID[i]
  # masked_nm
...
# 9881 unique behavioral scores remain after the filtering above
nrow(manual_scoring_nm)

# TKTK see below
# 4028 unique recording events remain after the filtering above
length(unique(manual_scoring_nm$Observation.id))

# Why are there only 4027 here??
unique_video_pats <- manual_scoring_nm %>% 
  pull(unmasked_video_file) %>% 
  gsub("_pre_trigger.mp4|_post_trigger.mp4", "", .) %>% 
  unique()

length(unique_video_pats)

# I think the discrepancy is because above I'm pulling the BORIS observation IDs and then I'm pulling the actual video file names. So this means that one recording event has two BORIS observation IDS, check this
# i <- 1
invisible(pblapply(1:length(unique_video_pats), function(i){
  ...
  if(tmp > 1){
    ...
    cat(paste(unique_video_pats[i], "\n", sep = ""))
    
  }
  
}))

# Box_03_2023_8_7_13_00_41 has two BORIS observation IDs

# TKTK drop this recording event for now, but check with Summer about what happened here

manual_scoring_nm <- manual_scoring_nm %>% 
    dplyr::filter(!grepl("Box_03_2023_8_7_13_00_41", unmasked_video_file))

glimpse(manual_scoring_nm)

# 9877 unique behavioral scores remain after the filtering above
nrow(manual_scoring_nm)

# 4026 unique BORIS observation files remain after the filtering above
length(unique(manual_scoring_nm$Observation.id))

# 4026 unique recording events remain after the filtering above, looks good
length(unique(manual_scoring_nm$unmasked_video_file))

```

# Obtain raw movement sensor data from ABISSMAL tracking system

Get the raw data across movement sensors collected by the ABISSMAL tracking system. This is the same dataset collected from 1 pair of birds reported in the current methods manuscript. This raw data will be indexed or filtered to retain only detections associated with the video recording events manually scored by Summer.

## Obtain raw video recording event data

Start by getting the raw data of video recording events.
```{r}
...
# In the BORIS manual scoring data, we can use the column of Media File Duration to extract the exact duration of each video in each unique recording event (note that there is lots of variability in the duration of the pre-movement video recorded using the ring buffer in the video recording module)

```

### Get the video start timestamps
...
# Looks good, the same after filtering above
length(unique_video_pats)
...
# This object now includes the start and end timestamps for each unique recording event (each pair of videos)
