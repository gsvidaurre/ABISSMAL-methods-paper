...
Notes on 05 Sep 2025:

- Doublecheck again, but all of the code in this script should now be contained across all other scripts

Notes on 23 May 2025:

- Next steps from last coding session: 
  - Fix unit test paths and run unit tests for ABISSMAL, then push and create a pull request for issue # 478 if tests pass
- Additional next steps:
  - The top priority: Think through the validation analyses to include in this manuscript.
  - Look into the issue I found with merging videos with other movement sensors - from my last coding session with the songbird team, I think I found an issue with my logic in how I was handling the unmasked videos (see notes below)
  - Check code that uses time lags against code development updates for a related project
...
# Path to the GitHub repository, to save lightweight files necessary for analyses that should be accessible across more than one machine
repo_path <- "~/Desktop/GitHub_repos/ABISSMAL-methods-paper"
...
# Write out these observation IDs as a .csv file, then read it back in below to keep using the same set of videos across computers
data.frame(random_sample = subsample) %>% 
  write.csv(file.path(repo_path, "randomSample_BORIS_Observations_codeDevelopment.csv"), row.names = FALSE)

```

Read these randomly sampled BORIS Observation IDs back into R.
```{r}

subsample <- read.csv(file.path(repo_path, "randomSample_BORIS_Observations_codeDevelopment.csv")) %>%
  pull(random_sample)
...
# Filter by the randomly selected subsample of BORIS Observations
...
# TKTK I think this is where the issue lies. Each row of the BORIS Observation file is not a unique video recording within a unique recording event. Rather, each row is a separate behavioral state within a video recording that is within a unique recording event. If any of the data processing or analyses below end up treating each row as an independent video file, then this is wrong. Consider changing this code to unmask BOTH masked video file names in the pair instead, which will likely require changes to the previous script (validation_pre-process_manualscores.Rmd). Actually, do this even if I was just using the unmasked name to extract metadata that applies to the whole unique recording event, to avoid confusion later 
...
TKTK Need to check the analyses below very carefully: given what we found with other manually scored videos, the analyses below should not be assuming that observations can be mapped onto unique videos with a pair, unless we match timestamps within an observation to timestamps within the duration of unique videos within a pair
...
recording_event_ts <- data.table::rbindlist(pblapply(1:length(unique_video_pats), function(i){
  ...
  start_post <- video_events %>% 
    ...
    pull(timestamp_ms) %>%
    # The timestamp value in the timestamp_ms column represents the moment when movement was detected, or the start time of the post-movement video, and this value is repeated for each video in a pair
    unique()
  
  if(length(start_post) == 1){
    ...
    # Create the start timestamp for this unique recording event by subtracting the duration of the first video in this event from the start timestamp of the post-motion video
    ...
    if(offset < 0.01){
      ...
      return(
        data.frame(
          ...
          end_event = end_event,
          duration_event = end_event - start_event
        )
      )
    ...
    stop("More than one timestamp was returned for the start of the post-movement video, which is not the expected behavior")
...
# The number of rows here represents the number of unique recording events and therefore should be the same number as the subsample of BORIS observations, looks good
...
For these analyses, we should collapse behavioral scores in the manual dataset across individuals in cases when the automated pipeline would not be able to separate individuals that performed the same behavior, whether concurrently or sequentially. The behavioral events for which we expect behaviors to occur concurrently are when more than one individual is assigned the behavioral state "INSC" or "inside container". ABISSMAL currently uses the presence of a video recording plus the absence of detections by other movement sensors to infer whether one or more birds was inside of the nest container. Currently, the ABISSMAL functions cannot discriminate whether one bird or more than one bird was inside of the container, nor the individual identity of the bird(s) inside of the container.
...
Plot the overall amount of activity per parameter combination while using the collapsed manual scoring dataset as a baseline. Across all videos.
```{r}
