...
th_1 <- c(2, 3, 4) # Threshold values of 0.1 and 0.2 yielded no perching events for RFID and IRBB. A value of 1 second did not yield perching events for the RFID data, so I dropped this too
...
# TKTK automated scores are missing for one video at times
...
# There is an RFID perching event detected at 7:49, so the issue is not that we're missing this information in the raw data or in the perching event detection but somehow this information is getting lost at the integration step. See below, there actually isn't an issue with integration, instead the RFID perching event is assigned to the next video recording event, and this makes sense looking at the seconds on the timestamps
rfid_perch %>% 
  View()

scored_clusters <- read.csv(file.path(path, "processed", "scored_detectionClusters_th1-2_th2-2_th3-0.1.csv"))  %>% 
  # Make sure that the timestamps are in the right format
  dplyr::mutate(
    start = as.POSIXct(format(as.POSIXct(start, tz = ""), "%Y-%m-%d %H:%M:%OS6")),
    end = as.POSIXct(format(as.POSIXct(end, tz = ""), "%Y-%m-%d %H:%M:%OS6"))
...
# INSC was collapsed into a single score across subjects, looks good
...
# Currently scored as inside of the container, as well as an entrance!!
combined_auto_results_extraAssignedVids %>% 
  dplyr::filter(video_file_name == "Box_03_2023_8_9_08_47_32_pre_trigger.mp4")

# One perching event detected at 08:59, but not close to 08:47
irbb_perch %>% 
  View()

# Same for the RFID perching events
rfid_perch %>% 
  View()

# There definitely are detections at 08:47:30 and 08:47:31, but these are not being caught by either the perching event detection or are likely being overfiltered during pre-processing
# These timestamps occur in the gap of time immediately before the start of post-movement video recording, or during the pre-movement recording
rfid_raw %>% 
  dplyr::filter(month == 8 & day == 9) %>% 
  View()

# The outer beam breaker has timestamps at 08:47.27 and 08:47.30, which also coincide with RFID
irbb_raw %>% 
  dplyr::filter(month == 8 & day == 9) %>% 
  View()

# Next, what happens if I don't filter out any detections in RFID and IRBB pre-processing? These detections still aren't caught when integrating data across sensors (detect_clusters() ).

# Next, try changing the temporal threshold for detect_clusters(). Is the run_length of 1 an inclusive minimum filter? These two detections are 1.2 seconds apart, so they should be caught using a temporal threshold of 2 seconds in detect_clusters()
# 08:47:30.357273
# 08:47:31.560623

# Changing the temporal threshold used for detection added back these timestamps to detect an entrance event! But the perching remains impacted, so next change the threshold used for detecting perching too. Then end troubleshooting
