...
combined_auto_results %>% 
  write.csv(file.path(path, "processed", paste("combined_indexed_automated_behavioral_inferences.csv", sep = "")))
...
TKTK consider whether each automatically identified event needs to be matched with a unique video recording event for this analysis.
```{r}

auto_scoring <- read.csv(file.path(indexed_path, "combined_indexed_automated_behavioral_inferences.csv")) %>%
  # Make sure that the timestamps are in the right format
  dplyr::mutate(
    start = as.POSIXct(format(as.POSIXct(start, tz = ""), "%Y-%m-%d %H:%M:%OS6")),
    end = as.POSIXct(format(as.POSIXct(end, tz = ""), "%Y-%m-%d %H:%M:%OS6"))
...
# In this plot, there should be more behavioral states scored manually than using the automated inferences (see my rationale above)
