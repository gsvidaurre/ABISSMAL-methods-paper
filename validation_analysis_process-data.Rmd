...
th_1_1 <- c(1, 2, 3, 4) # RFID perching threshold. Threshold values of 0.1 and 0.2 yielded no perching events for RFID and IRBB.

th_1_2 <- c(1, 2, 3, 4) # IRBB perching threshold value.

# Temporal thresholds in seconds for preprocess_detections(), RFID and IRBB data
th_2 <- c(0, 2, 3, 4) # Threshold values of 0.1 and 0.2 yielded no clusters of detections that should be filtered. A value of 1 second did not yield clusters of detections that should be filtered for the RFID data, so I dropped this too
...
# Testing
x <- 2 # RFID and IRBB perching threshold of 2 seconds
y <- 1 # pre-processing threshold of 0, or no filtering
z <- 4 # detect_cluster threshold of 2 seconds
  ...
  invisible(pblapply(1:length(th_1), function(x){
    ...
    lapply(1:length(th_2), function(y){
      ...
      lapply(1:length(th_3), function(z){
        ...
        cat(paste("th_1_1 =", th_1[x], ": th_1_2 =", th_1[x], "; th_2 =", th_2[y], "; th_3 =", th_3[z], "\n", sep = " "))
        
        # TKTK this function should be detecting a perching event on 08-09 08:47:30 and 08:47:31
        # This is definitely a problem with how the perching events are being detected
        detect_perching_events(file_nm = "combined_raw_data_RFID.csv", threshold = th_1_1[x], run_length = 1, sensor_id_col_nm = "sensor_id", timestamps_col_nm = "timestamp_ms", PIT_tag_col_nm = "PIT_tag_ID", rfid_label = "RFID", outer_irbb_label = NULL, inner_irbb_label = NULL, general_metadata_cols = c("chamber_id", "sensor_id"), path = path, data_dir = "raw_combined", out_dir = "processed", out_file_prefix = "perching_events", tz = "", POSIXct_format = "%Y-%m-%d %H:%M:%OS")
        
        detect_perching_events(file_nm = "combined_raw_data_IRBB.csv", threshold = th_1_2[x], run_length = 2, sensor_id_col_nm = "sensor_id", timestamps_col_nm = "timestamp_ms", PIT_tag_col_nm = NULL, rfid_label = NULL, outer_irbb_label = "Outer Beam Breaker", inner_irbb_label = "Inner Beam Breaker", general_metadata_cols = c("chamber_id", "sensor_id"), path = path, data_dir = "raw_combined", out_dir = "processed", out_file_prefix = "perching_events", tz = "", POSIXct_format = "%Y-%m-%d %H:%M:%OS")
        
        # Pre-process RFID and IRBB and integrate from the pre-processed files if th2 is greater than 0, otherwise use the raw combined data as the input for integration and scoring
        if(th_2[y] > 0){
          ...
          # detect clusters for 3 sensor types
          detect_clusters(file_nms = c("combined_raw_data_IRBB.csv", "combined_raw_data_RFID.csv", "combined_raw_data_Video.csv"), threshold = th_3[z], run_length = 1, sensor_id_col_nm = "sensor_id", timestamps_col_nm = "timestamp_ms", PIT_tag_col_nm = "PIT_tag_ID", rfid_label = "RFID", camera_label = "Camera", preproc_metadata_col_nms = c("thin_threshold_s", "pixel_threshold", "data_stage", "date_pre_processed"), general_metadata_col_nms = c("chamber_id", "year", "month", "day"), video_metadata_col_nms = c("total_pixels_motionTrigger", "video_file_name"), path = path, data_dir = "raw_combined", out_dir = "processed", out_file_nm = "detection_clusters.csv", tz = "", POSIXct_format = "%Y-%m-%d %H:%M:%OS")
        ...
        out_file_nm <- paste(paste("scored_detectionClusters", paste("th1", th_1_1[x], sep = "-"), paste("th2", th_2[y], sep = "-"), paste("th3", th_3[z], sep = "-"), sep = "_"), ".csv", sep = "")
...
Pull apart the different behavioral inferences that are possible here using automated ABISSMAL integration, including perching events here.
```{r}

# files <- list.files(file.path(path, "processed"), pattern = "indexedscored_")
# length(files)

files <- list.files(file.path(path, "processed"), pattern = "indexedscored_detectionClusters_th1-2_th2-0_th3-2.csv")
# files
...
combined_auto_results <- data.table::rbindlist(pblapply(1:length(files), function(i){
  ...
  res <- read.csv(file.path(path, "processed", files[i]))  %>% 
    ...
    dplyr::select(-c("X")) %>% 
    # Extract automated behavioral inferences
    dplyr::mutate(perching_inference = ifelse(!is.na(perching_rfid_start) | !is.na(perching_outer_irbb_start) | !is.na(perching_inner_irbb_start), "perching", NA)) %>% 
    dplyr::mutate(
      inferences = paste(direction_scored, inferredMovement_Location, perching_inference, sep = "-")
    ) %>%
    # Split inferences across columns
    separate(data = ., col = inferences, sep = "-", into = c("inference_1", "inference_2", "inference_3")) %>% 
    dplyr::select(-c("perching_inference")) %>% 
    # Pivot longer so that each inference per video is a separate row
    pivot_longer(data = ., cols = c("inference_1", "inference_2", "inference_3"), names_to = "inference_number", values_to = "inference") %>% 
    # Remove NA strings from the new inferences column
    dplyr::filter(inference != "NA") %>% 
    # Remove "container_entrance" that originally came from the inferredMovement_Location column values. We only need that column to specify when the inference was that a bird was inside of the nest container
    dplyr::filter(
      inference != "container_entrance"
    )
...
# TKTK NEXT STEPS AND TKTK CONTINUE
# DONE: Figure out why this video is not included in the automated scores. FIXED this with the logic of how I was searching for video timestamps that matched scored inferences (same logic fix for perching events)
# NEXT: Fix unit test paths and run unit tests for ABISSMAL, then push and create a pull request for issue # 478 if tests pass 

# There were strikingly few and similar number of rows here per parameter combination, which generally means that the overall amount of activity detected should be pretty similar across parameter combinations. Curerntly this is very likely due to the fact that I'm using a small subsample of videos to develop this code
...
Proceed by collapsing the INSC scores in the manual dataset to a single event across individuals. Collapse these scores across individuals even if the timestamps of the INSC events do not fully overlap, since this is a very coarse-grained analysis. Also remove NOBR or no bird present scores, since ABISSMAL functions currently do not make inferences about whether no birds are present inside of the container.
```{r}
...
manual_scoring_nm_coll <- manual_scoring_nm %>%
  # First remove the subject information
  ...
  bind_rows(
    manual_scoring_nm %>%
      dplyr::filter(Behavior != "INSC") %>% 
      # Also remove all NOBR scores
      dplyr::filter(Behavior != "NOBR") %>% 
      dplyr::select(unmasked_video_file, Behavior)
  ) %>% 
...
For preliminary analyses with 50 randomly sampled video recording events, automated scoring consistently identified 17 events less than manual scoring (80 events total, compared to 63 events identified by automated processing with different parameter combinations). Currently these results are for a single combination of automated processing parameters.
...
manual_scoring_nm_summ <- manual_scoring_nm_coll %>% 
  group_by(unmasked_video_file) %>% 
  dplyr::summarise(
    n_events = n()
  ) %>% 
...
# Not seeing automated scores are missing for any videos in the subsample
...
gg_res_wide %>% 
  ...
  scale_y_continuous(limits = c(0, max(gg_res_wide$manual_scoring) + 0.1), breaks = seq(0, max(gg_res_wide$manual_scoring), 1)) +
  scale_x_continuous(limits = c(0, max(gg_res_wide$manual_scoring) + 0.1), breaks = seq(0, max(gg_res_wide$manual_scoring), 1)) +
...
Find videos for doublechecking which behavioral the automated approach is missing.
```{r}

gg_res %>% 
  pivot_wider(id_cols = c(video_file_name, method), names_from = type, values_from = "n_events") %>%
  dplyr::filter(manual_scoring == 2) %>% 
  slice(1) %>% 
  pull(video_file_name)

# "Box_03_2023_7_22_06_14_17_pre_trigger.mp4"

# The INSC behaviors were collapsed into a single score across subjects, looks good
# PERC has 1.52 second duration
manual_scoring_nm %>% 
  dplyr::filter(unmasked_video_file == "Box_03_2023_7_22_06_14_17_pre_trigger.mp4")

# INSC, then PERC
manual_scoring_nm_coll %>% 
  dplyr::filter(unmasked_video_file == "Box_03_2023_7_22_06_14_17_pre_trigger.mp4")

# Check raw RFID and IRBB data around this video recording timestamp
# No RFID detections on this date around 6:14
rfid_raw <- read.csv(file.path(sensor_path, "combined_raw_data_RFID.csv"))
glimpse(rfid_raw)

# There is a detection at 6:14 by the outer beam breaker, but only a single detection, so this wouldn't have been picked up as a "long" perching event as detected by the ABISSMAL integration functions
# Consider another filter on the perching event duration
irbb_raw <- read.csv(file.path(sensor_path, "combined_raw_data_IRBB.csv"))
glimpse(irbb_raw)

rfid_raw %>% 
  dplyr::filter(month == 7 & day == 22) %>% 
  View()
...
gg_res %>% 
  pivot_wider(id_cols = c(video_file_name, method), names_from = type, values_from = "n_events") %>%
  dplyr::filter(manual_scoring == 4) %>% 
  slice(1) %>% 
  pull(video_file_name)

# "Box_03_2023_7_5_07_49_14_pre_trigger.mp4"

# INSC, ENTR, PERC, EXIT
manual_scoring_nm %>% 
  dplyr::filter(unmasked_video_file == "Box_03_2023_7_5_07_49_14_pre_trigger.mp4")
manual_scoring_nm_coll %>% 
  dplyr::filter(unmasked_video_file == "Box_03_2023_7_5_07_49_14_pre_trigger.mp4")

# Scored as INSC only when the IRBB perching threshold is 2 seconds or less, but an IRRB perching event is detected (overly long) when the IRBB perching threshold is 30 seconds (very long). To me that means that the functions are working as expected, but unfortunately, when only one beam breaker detected movement and the RFID antenna failed, we will not have good resolution of events like these (entrance, short perching event, exit by one bird while another is inside of the nesting container)
combined_auto_results_extraAssignedVids %>% 
  dplyr::filter(video_file_name == "Box_03_2023_7_5_07_49_14_pre_trigger.mp4")


# RFID detections at 7:49 began at 07:49:37.275614 
rfid_raw %>% 
  dplyr::filter(month == 7 & day == 5) %>% 
  View()

# IRBB detections at 7:49 began at 07:49:12.439107
irbb_raw %>% 
  dplyr::filter(month == 7 & day == 5) %>% 
  View()

# RFID perching events
rfid_perch <- read.csv(file.path(path, "processed", "perching_events_RFID.csv"))
glimpse(rfid_perch)

irbb_perch <- read.csv(file.path(path, "processed", "perching_events_IRBB.csv"))
glimpse(irbb_perch)

# There is an RFID perching event detected at 7:49, so the issue is not that we're missing this information in the raw data or in the perching event detection but somehow this information is getting lost at the integration step. Actually see below, there isn't an issue with integration, instead the RFID perching event is assigned to the next video recording event, and this makes sense looking at the seconds on the timestamps. The problem is that the RFID antennae failed to detect a PIT tag around the same time that the outer IRBB detected movement. Try one more round of data processing above that varies the threshold for detecting perching events for IRBB only (make this longer to merge detections that occurred within the same minute into perching events) 
# 2023-07-05 07:49:37.275614 to 2023-07-05 07:49:45.134487
rfid_perch %>% 
  View()

scored_clusters <- read.csv(file.path(path, "processed", "scored_detectionClusters_th1-2_th2-0_th3-2.csv"))  %>% 
  # Make sure that the timestamps are in the right format
  dplyr::mutate(
    start = as.POSIXct(format(as.POSIXct(start, tz = ""), "%Y-%m-%d %H:%M:%OS6")),
    end = as.POSIXct(format(as.POSIXct(end, tz = ""), "%Y-%m-%d %H:%M:%OS6"))
...
# View(scored_clusters)

# How long is the perching event identified by manual scoring? 4.12 seconds
manual_scoring_nm %>% 
  dplyr::filter(unmasked_video_file == "Box_03_2023_7_5_07_49_14_pre_trigger.mp4") %>% 
  View()
...
# gg_res %>% 
#   pivot_wider(id_cols = c(video_file_name, method), names_from = type, values_from = "n_events") %>%
#   dplyr::filter(manual_scoring == 5) %>% 
#   slice(1) %>% 
#   pull(video_file_name)

# "Box_03_2023_8_9_08_47_32_pre_trigger.mp4"

# The perching event durations are really short, around 0.7 and 0.2 seconds each
manual_scoring_nm %>% 
  dplyr::filter(unmasked_video_file == "Box_03_2023_8_9_08_47_32_pre_trigger.mp4")

# NOBR was removed, looks good
manual_scoring_nm_coll %>% 
  dplyr::filter(unmasked_video_file == "Box_03_2023_8_9_08_47_32_pre_trigger.mp4")

# Currently scored as inside of the container, as well as an entrance, and a perching event!!
# TKTK need to remove the location "container_entrance", since that column should only be used for the inference "inside_container"
combined_auto_results_extraAssignedVids %>% 
  dplyr::filter(video_file_name == "Box_03_2023_8_9_08_47_32_pre_trigger.mp4")

# Scored_clusters should have caught the RFID perching event below because it occurs within the timestamps of the behavioral event, but it is not integrated here. In addition, the camera event that starts at 2023-08-09 08:47:32 appears as a separate row here but was already integrated into the first row. But this is the expected behavior actually. The issue is that perching events are NOT integrated but I know that an RFID perching event was detected. YES FIXED IT
# The next issue to fix is that technically this video is now associated with THREE behavioral inferences using the automated approach (inside container, entrance, and perching)
scored_clusters %>% 
  dplyr::filter(start >= "2023-08-09 08:47:00", start <= "2023-08-09 08:48:00")

# No perching event is detected by IRBB around 7:49, using a threshold of 2 seconds, which makes sense because these detections were spread out over about 10 seconds
irbb_perch %>% 
  # dplyr::filter(perching_start == "2023-08-09 08:47:30.029577")
  View()

# Here the perching event I'm looking for starts is 2023-08-09 08:47:30.357273
# This does appear and also has the correct duration too
rfid_perch %>% 
  dplyr::filter(perching_start == "2023-08-09 08:47:30.357273")
# View()
