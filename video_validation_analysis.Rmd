...
# There should be 1 row per unique video recording event, looks good
glimpse(tp_fp_tn_fn)
# View(tp_fp_tn_fn)
...
Generall approach to calculating individual identity detection performance:

**True positive** = The same individual(s) were detected by both ABISSMAL and manual scoring.

**False negative** = ABISSMAL detected either no PIT tag or the incorrect PIT tag compared to the individual identity detected by manual scoring. 

**False positive** = ABISSMAL detected a PIT tag or the incorrect PIT tag compared to the individual identity detected by manual scoring, which detected .
...
TKTK update the code below and modify for individual identity detection.
```{r}
...
5. BEHAVIORAL INFERENCE: Next, ask how well the integrated dataset of movement sensors scored the TYPE of activity detected. This is a finer-grained validation to ask if behavioral categories associated with the video recording events match between the two methods. Check out my notes above that detail expectations - there will be more scores per video recording event using the manual method.
