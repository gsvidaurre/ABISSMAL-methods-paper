...
# TKTK the same number of rows should be here compared to the number of videos in the subsample
...
Score detection clusters to make inferences about behavioral events associated with these movements. TKTK need to later integrate perching events (), also sensor_id_col_nm, PIT_tag_col_nm cannot be NULL
...
1. OVERALL RECALL IN ACTIVITY DETECTION: First, how well does the integration of 3 movement sensors capture whether birds are inside of the container or not? Can we generally use the automated scores to infer that sensor activity was due to the birds or not? For this analysis, perform integration of data from the 3 sensors for all days of data collection, and then index the scored activities using the timestamps of the manually scored video recording events with a time buffer around them. 

General approach to calculating detection performance:

For the binary analysis below, we want to know how often ABISSMAL and manual scoring picked up activity versus no activity, regardless of the behavior itself, the individual, or the location (container entrance or inside of the container).

**True positive** = activity detected by both ABISSMAL and manual scoring. How often did ABISSMAL capture activity when activity was also picked up by manual scoring (any of PERC, ENTR, EXIT, INSC)?

**False negative** = ABISSMAL detects activity, but manual scoring detected a lack of activity. How often did manual scoring find no birds inside of the container for the duration of the video recording event (NOBR) but ABISSMAL detected activity (any of PERC, ENTR, EXIT, INSC)?

**False positive** = activity detected by ABISSMAL but not manual scoring (we cannot assess this without continuous video recording)

**True negative** = neither ABISSMAL nor manual scoring detect activity (we cannot assess this without continuous video recording)

We can assess the rate of true positives and the rate of false negatives, which facilitates calculating the recall, or the true positive rate, which is: 
...
# TKTK need to doublecheck: `score_clusters` should be making inferences by just the first edge encountered, not all edges in a sequence, because the system is currently targeted to capturing overall patterns of activity
...
Then: true positives / (true positives + false negatives)
...
tp_fn <- data.table::rbindlist(pblapply(1:length(unique_videos), function(i){
...
Calculate recall of overall activity detection, which for the current subsample of 50 video recording events was 98.04%.
```{r}
...
recall <- round((tp_fn_sum$tps / (tp_fn_sum$tps + tp_fn_sum$fns)) * 100, 2)
recall

```


2. LOCATION RECALL: Next, focus on asking whether the automated and manual scores label activity by birds at the container entrance and/or inside of the container.

The options here are: container entrance, inside or container, or both. If we do a recall analysis, then we need to calculate true positives and false negatives for each of these classes. 
...
# TKTK currently this is not coded right for both locations
...
TKTK the recall calculation for both locations is not right because the logic above for asking if both labels from both locations are represented is wrong. FIX

Calculate recall of the local of the overall activity detection, which for the current subsample of 50 video recording events was container entrance: 50%, inside container: 88%, and both locations: 100%.
```{r}
...
recall_ce <- round((tp_fn_sum %>% 
                   dplyr::filter(location == "container_entrance") %>% 
                   pull(tps) / (
                     tp_fn_sum %>% 
                   dplyr::filter(location == "container_entrance") %>% 
                   pull(tps) + 
...
recall_ic <- round((tp_fn_sum %>% 
                   dplyr::filter(location == "inside_container") %>% 
                   pull(tps) / (
                     tp_fn_sum %>% 
                   dplyr::filter(location == "inside_container") %>% 
                   pull(tps) + 
...
recall_bo <- round((tp_fn_sum %>% 
                   dplyr::filter(location == "both") %>% 
                   pull(tps) / (
                     tp_fn_sum %>% 
                   dplyr::filter(location == "both") %>% 
                   pull(tps) + 
...
3. INDIVIDUAL IDENTITY RECALL: Next, ask how reliably we captured individual identity with RFID compared to the manually scored videos, and how often no PIT tags or only a single PIT was detected when manual video scoring detected 1 tagged adult or 2 inside of the nest container


4. Next, ask how well the integrated dataset of movement sensors agrees with the first behavioral score attributed to each video recording event by manual scoring. This is a finer-grained validation to ask if behavioral categories associated with the video recording events match between the two methods. Check out my notes above that detail expectations - there will be more scores per video recording event using the manual method.

Or, TKTK instead focus on asking whether the automated label is found anywhere in the manually scored sequence?


TKTK need to select the first label per sequence per video, then compare these.

Build a confusion matrix to visualize the labeling by manual scoring and automated inferences.
```{r}

table(manual_scoring_nm$Behavior, auto_scoring$inferences)
